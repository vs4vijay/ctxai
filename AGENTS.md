...
- **Retrieval**: `repository_context.py` — `discover_repository_indexes`, `ContextAssembler` (dedup + token budget), `HybridRetriever` (hybrid beats vector-only per vs03 tests). `evals/` package (RE-01) provides the versioned retrieval benchmark (`tests/fixtures/retrieval_benchmark.json`), the `ctxai eval retrieval` runner/metrics, and the shared eval artifact discipline (fingerprints, redaction, baseline gates) reused by HH-09.
...
- **mypy covers only explicit files** (`[tool.mypy] files`, currently ~18 incl. the `evals/` package): `agent/llm/contract.py`, `agent/tools/execution.py`, `agent/workflow.py`, `index_manifest.py`, `repository_context.py`, `evals/*`. New files aren't type-checked unless added there.
...
- **Gotchas**: root-level `test_*.py` files (`test_agent.py`, `test_copilot.py`, `test_oauth.py`, `test_openrouter.py`, `test_quick.py`) are **not collected** and some hit live APIs — don't run or extend them. `--strict-markers` errors on undeclared markers — add new ones to `pytest.ini`. MCP tests guard with `pytest.importorskip("mcp")`. `test_retrieval_eval.py` asserts the shipped benchmark's schema invariants (20 cases, valid splits, expected files exist in the repo) and the eval runner's behavior — update it when touching retrieval or the `evals/` package. Keep tests 3.10-compatible and warning-free.
