...
def list_config(project_path: Optional[Path] = None):
    """
    ...
    table.add_row("embedding.model", str(config.embedding.model) if config.embedding.model else "[dim]default[/dim]")
    ...
    table.add_row("embedding.max_tokens", str(config.embedding.max_tokens) if config.embedding.max_tokens else "[dim]not set[/dim]")
...
def get_config(key: str, project_path: Optional[Path] = None):
    """
...
def set_config(key: str, value: str, project_path: Optional[Path] = None):
    """
    ...
    try:
        ...
        if section == "embedding":
            ...
            if not hasattr(config.indexing, setting):
                ...
                console.print("[yellow]Available settings:[/yellow] max_files, max_total_size_mb, max_file_size_mb, chunk_size, chunk_overlap\n")
                return
            ...
            if setting == "name":
                ...
                console.print("[yellow]Only 'index.name' can be set manually. Other index metadata is set automatically during indexing.[/yellow]\n")
                return
        else:
...
def unset_config(key: str, project_path: Optional[Path] = None):
    """
    ...
    try:
        ...
        if section == "embedding":
            ...
            if setting == "name":
                ...
                console.print(f"[green]✓[/green] Cleared all index metadata\n")
                console.print(f"[dim]Config saved to: {config_manager.config_path}[/dim]\n")
                return
            else:
            ...
            console.print("[yellow]Only embedding.api_key, embedding.model, embedding.max_tokens, and index.name can be unset[/yellow]\n")
            return
...
def show_config_file(project_path: Optional[Path] = None):
    """
...
def edit_config(project_path: Optional[Path] = None):
    """
