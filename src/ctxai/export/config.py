...
class ExportConfig:
    ...
    def validate(self) -> None:
        ...
        if self.output_format not in self._ALLOWED_FORMATS:
            raise ValueError(f"output_format must be one of {self._ALLOWED_FORMATS}, got {self.output_format!r}")

    def matches_include(self, path: Path) -> bool:
        if not self.include_patterns:
            return True
        return any(fnmatch.fnmatch(path.name, p) or fnmatch.fnmatch(str(path), p) for p in self.include_patterns)
