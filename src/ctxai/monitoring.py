...
class MetricsRegistry:
    ...
    def snapshot(self) -> dict[str, dict]:
        with self._lock:
            return {
                "counters": {
                    f"{name}{self._format_labels(labels)}": value for (name, labels), value in self._counters.items()
                },
                "gauges": {
                    f"{name}{self._format_labels(labels)}": value for (name, labels), value in self._gauges.items()
                },
                "histograms": {
