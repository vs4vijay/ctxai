...
class ProjectSizeValidator:
    ...
    def validate(self, stats: ProjectStats) -> Tuple[bool, List[str]]:
        ...
        if stats.total_files > self.config.max_files:
            is_valid = False
            messages.append(
                f"❌ Too many files: {stats.total_files} files "
                f"(limit: {self.config.max_files})"
            )
            messages.append(
                f"   Consider using --include patterns to filter files, "
                f"or increase max_files in .ctxai/config.json"
            )
        elif stats.total_files > self.config.max_files * 0.8:
            # Warning at 80% of limit
            messages.append(
                f"⚠️  Approaching file limit: {stats.total_files} files "
                f"(limit: {self.config.max_files})"
            )
        ...
        if stats.total_size_mb > max_size_mb:
            is_valid = False
            messages.append(
                f"❌ Project too large: {stats.format_size(stats.total_size_bytes)} "
                f"(limit: {max_size_mb} MB)"
            )
            messages.append(
                f"   Consider using --include patterns to filter files, "
                f"or increase max_total_size_mb in .ctxai/config.json"
            )
        elif stats.total_size_mb > max_size_mb * 0.8:
            # Warning at 80% of limit
            messages.append(
                f"⚠️  Approaching size limit: {stats.format_size(stats.total_size_bytes)} "
                f"(limit: {max_size_mb} MB)"
            )
    ...
    def get_summary(self, stats: ProjectStats) -> List[str]:
        ...
        if stats.largest_files:
            messages.append(f"   • Largest files:")
