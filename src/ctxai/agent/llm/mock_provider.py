"""
Mock LLM provider for deterministic agent testing and benchmark runs.

This mock provider returns predefined responses instead of making real API
calls, enabling fast, deterministic testing without costs. It lives in the
package (moved from the test suite by HH-09) so the agent task benchmark's
mock-provider mode works from any installed ctxai, including clean
installations without the test suite.
"""
...
"""Default model id reported by the mock provider (unpriced in the cost table)."""
