...
class SessionManager:
    ...
    async def send_message(self, session_id: str, message: str) -> str:
        ...
        if session.agent.context.get_message_count() > self.max_messages_per_session:
            raise RuntimeError(f"Session has exceeded the max-messages limit ({self.max_messages_per_session})")
    ...
    def _build_agent(self, config: AgentConfig, tool_registry: ToolRegistry | None = None) -> Agent:
