...
class SecurityManager:
    ...
    def validate_bash_command(self, command: str) -> str:
        ...
        if len(command) > self.policy.max_command_length:
            raise SecurityError(
                f"Command too long: {len(command)} > {self.policy.max_command_length}"
            )
        ...
        if self.policy.allowed_commands is not None:
            ...
            if not any(head == cmd.lower() or normalized.startswith(cmd.lower())
                       for cmd in self.policy.allowed_commands):
                raise SecurityError(
                    f"Command not in whitelist (head='{head}'): "
                    f"{self.policy.allowed_commands}"
                )
